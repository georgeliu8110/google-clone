    ...
    <div className='sm:pb-24 pb-40 mt-4'>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 px-3 space-x-4'>
         {results.items.map(result=>
           <div key={result.id} className='mt-8'>
             <div className='group'>
                <Link href={result.image.contextLink}>
                  <img src={result.link} alt={result.title} className='h-69 group-hover:shadow-xl w-full object-contain transition-shadow duration-300' ></img>
                </Link>
                <Link href={result.image.contextLink}>
